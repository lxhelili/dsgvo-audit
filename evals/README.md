...
When the change is to an existing version, the baseline is the *previous* skill, not "no skill" — that answers the question that matters for a release: did this edit make audits better or worse?

### Run hygiene (learned in iterations 2–3)

- **Both configurations read from snapshots, never from the working tree.** `git archive <tag> skills/dsgvo-audit | tar -x -C evals/workspace/skill-snapshot-<version> --strip-components=2` for the candidate *and* the baseline. Iteration 3 edited `skills/dsgvo-audit/` while a with-skill run was still reading it; that run had to be discarded and repeated. With snapshots, fixing what the first graders report cannot leak into runs that are still going.
- **Executors never see the grading key.** Give them the prompt and the file list inline; tell them not to read `evals/evals.json`, `evals/workspace/**` outside their snapshot and output folder, or `scripts/grade-report.mjs` (one iteration-3 run read the grader and graded itself). `eval_metadata.json` is for the graders.
- **Repeat runs.** One run per configuration cannot separate a skill change from variance. Use `<eval>/<config>/run-1/`, `run-2/` … (each with `outputs/`, `transcript.md`, `grading.json`, `timing.json`); `summarize-evals.mjs` sums the pass counts, averages time and tokens, and lists **flaky expectations** — passed in some runs, failed in others. Two runs per configuration is the minimum for a release decision; treat a ±1 difference on a single eval as noise unless it is the same in every run.
- **One grader per eval** grades all runs of both configurations, applying the same reading of each expectation to every run, and says how it treated borderline cases (e.g. provider addresses from general knowledge).
- **Serve fixtures on distinct ports** per parallel run (eval 2).
- **Run the deterministic grader after fixing it**, and say so in the results file — iterations 2 and 3 each found grader false positives on correct output.
