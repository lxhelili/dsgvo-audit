...
- **Impressum** from `assets/impressum-template.md`. Only what the client or a register states goes into the file — never a Kammer, Aufsichtsbehörde, register court or title inferred from the location or trade; an unknown mandatory item goes into the open questions, not into the text. The same for the DSE: a provider address taken from general knowledge, or a technical statement the code does not show ("ausschließlich über HTTPS/TLS"), may stay in the text only if your message lists it under „vor Veröffentlichung prüfen“.
